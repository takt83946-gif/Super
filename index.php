<?php
/* ==========================================================
   1. الاتصال بقاعدة البيانات ومعالجة إرسال الإعلانات (Wish)
   ========================================================== */
...
<head>
    ...
    <style>
        ...
        .modal-content { background-color: white; margin: 4% auto; padding: 25px; border-radius: 14px; width: 90%; max-width: 500px; box-shadow: 0 5px 25px rgba(0,0,0,0.3); max-height: 85vh; overflow-y: auto; }
...
<div class="search-container">
    <input type="text" id="searchInput" class="search-input" placeholder="🔍 ابحث عن أي منتج..." onkeyup="filterProducts()">
</div>

<div class="container">
    <!-- شبكة إعلانات الزباين (Bento Grid) المعتمدة -->
    ...
    <div class="ads-bento-grid">
        ...
        if ($ads_res &&$ads_res->num_rows > 0) {
            while ($ad =$ads_res->fetch_assoc()) {
                ...
                echo '<a href="https://wa.me/961' . preg_replace('/[^0-9]/', '', $ad['phone']) . '" target="_blank" style="background:#25d366; color:white; padding:4px 10px; border-radius:4px; text-decoration:none; font-weight:bold;">تواصل واتساب</a>';
                echo '</div>';
                echo '</div>';
            }
        } else {
...
<!-- Modal إضافة إعلان مع Wish Money -->
<div id="adModal" class="modal">
    <div class="modal-content">
        ...
        <div class="wish-box">
            ...
            حوّل رسم الإعلان إلى رقم Wish: <b>03-000000 (Ali Store)</b>، ثم أدخل رقم سند الحوالة أدناه ليتم اعتماد إعلانك فوراً بعد المراجعة.
        </div>
        <form method="POST">
            ...
            <div class="form-group"><label>رقم الهاتف:</label><input type="text" name="ad_phone" required></div>
...
<!-- Modal السلة (نفس المنطق السابق) -->
<div id="cartModal" class="modal">
    <div class="modal-content">
        <span class="close-btn" onclick="toggleCartModal()">&times;</span>
        <h2 style="color:var(--primary-color);">🛒 سلة المشتريات</h2>
        <ul id="cart-items" style="list-style:none; padding:0; max-height:220px; overflow-y:auto;"></ul>
        <div style="font-weight:bold; margin:15px 0;">المجموع: <span id="cart-total">0</span> ليرة</div>
        <button class="btn" style="background:#25d366;" onclick="sendToWhatsApp()">إرسال الطلب عبر واتساب 📱</button>
    </div>
</div>
...
function updateCart() {
    let c = document.getElementById('cart-count'), l = document.getElementById('cart-items'), t = document.getElementById('cart-total');
    let cnt = 0, tot = 0; l.innerHTML = '';
    cart.forEach(i => { cnt += i.quantity; tot += i.price * i.quantity; l.innerHTML += `<li>${i.name} (x${i.quantity}) - ${i.price*i.quantity} ليرة</li>`; });
    c.innerText = cnt; t.innerText = tot;
}
updateCart();
